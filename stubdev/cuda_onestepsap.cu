...
__device__ void CalcConstraintCost(SAPGPUData* data) {
  ...
  for (int i = threadIdx.x; i < data->num_active_contacts() * 3;
       i += blockDim.x) {
    if (i % 3 == 2) {
      float& k = data->contact_stiffness(i / 3)(0, 0);
      float& d = data->contact_damping(i / 3)(0, 0);
      float& phi_0_i = data->phi0(i / 3)(0, 0);
      float v_d = 1.0 / (d + 1.0e-20);
      float v_x = data->phi0(i / 3)(0, 0) * k / dt / (k + 1.0e-20);
      ...
      // float v_n = (data->J().row(i) * data->v_guess())(0, 0);
