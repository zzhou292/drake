...
struct SAPGPUData {
  ...
  __device__ Eigen::Map<Eigen::Vector3d> v_gamma(int constraint_index) {
    return Eigen::Map<Eigen::Vector3d>(
        v_gamma_global + (blockIdx.x * num_contacts + constraint_index) * 3, 3,
        1);
  }

  __device__ Eigen::Map<Eigen::Vector3d> v_R(int constraint_index) {
    return Eigen::Map<Eigen::Vector3d>(
  ...
  __host__ __device__ const int NumEquations() const { return num_problems; }

  // Retrival functions - copy data back to CPU
  void GetLambdaM(std::vector<double>& lambdam) {
    lambdam.resize(num_problems);
    cudaMemcpy(lambdam.data(), lambdam_global, num_problems * sizeof(double),
               cudaMemcpyDeviceToHost);
  }

  // Retrival functions - copy data back to CPU
  void GetLambdaR(std::vector<double>& lambdar) {
    lambdar.resize(num_problems);
    cudaMemcpy(lambdar.data(), lambdar_global, num_problems * sizeof(double),
               cudaMemcpyDeviceToHost);
  }

  //   // Retrival functions - copy data back to CPU
  //   void GetDeltaP(std::vector<Eigen::MatrixXd>& deltap) {
  //     deltap.resize(num_problems);
  //     for (int i = 0; i < num_problems; i++) {
  //       Eigen::MatrixXd temp(3 * num_velocities, 1);
  //       cudaMemcpy(temp.data(), delta_p_global + i * 3 * num_velocities,
  //                  3 * num_velocities * sizeof(double),
  //                  cudaMemcpyDeviceToHost);
  //       deltap[i] = temp;
  //     }
  //   }
  ...
  //     deltav.resize(num_problems);
  //     for (int i = 0; i < num_problems; i++) {
  //       Eigen::MatrixXd temp(3 * num_velocities, 1);
  //       cudaMemcpy(temp.data(), delta_v_global + i * 3 * num_velocities,
  //                  3 * num_velocities * sizeof(double),
  //                  cudaMemcpyDeviceToHost);
  //       deltav[i] = temp;
  //     }
  ...
  void MakeSAPGPUData(std::vector<SAPCPUData> data) {
    ...
    HANDLE_ERROR(cudaMalloc(&v_R_global,
                            num_problems * num_contacts * 3 * sizeof(double)));

    HANDLE_ERROR(cudaMalloc(&delta_p_global, num_problems * 3 * num_velocities *
                                                 3 * num_velocities *
                                                 sizeof(double)));
    HANDLE_ERROR(cudaMalloc(&lambdam_global,
                            num_problems * sizeof(double)));  // 1D vector

    HANDLE_ERROR(cudaMalloc(&lambdar_global, num_problems * sizeof(double)));
    ...
    for (int i = 0; i < num_problems; i++) {
      ...
      for (int j = 0; j < num_contacts; j++) {
        HANDLE_ERROR(cudaMemcpy(v_gamma_global + i * num_contacts * 3 + j * 3,
                                ...
                                cudaMemcpyHostToDevice));
        HANDLE_ERROR(cudaMemcpy(v_R_global + i * num_contacts * 3 + j * 3,
                                data[i].v_R[j].data(), 3 * sizeof(double),
                                cudaMemcpyHostToDevice));
      }
    }
  }
